...
历史按私聊用户或“群 + 用户”隔离，只把完整问答轮次发送给模型。`BOT_CHAT_CONTEXT_MAX_CHARS` 控制历史总字符预算；超出预算时，旧轮次会被本地提取式压缩为受限节选，最近轮次保留原文。`BOT_CHAT_CONTEXT_TTL_MINUTES` 按最后活跃时间让闲置会话整体过期，设为 `0` 可关闭自动过期。明确以“换个话题”“忽略之前的对话”“重新开始”等表达开头时，当前会话从新话题重新开始；普通转折表达不会触发清空。`/上下文状态` 不返回聊天正文，`/上下文摘要` 只返回当前会话完整轮次的本地节选，`/清空上下文` 只清理当前会话。聊天历史不会自动成为知识库或长期记忆。
