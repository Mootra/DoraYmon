...
DoraYmon 是一个插件化 Python QQ AI 助手，使用 `botpy` 接入 QQ，通过 DeepSeek 提供大模型对话，并支持默认关闭、按会话隔离的可控短期上下文。当前意图识别是规则匹配；RAG、Embedding、长期个人记忆和 Agent 尚未实现。
...
- 本地知识库 RAG、来源引用、Embedding、向量检索、长期个人记忆、Agent 和模型训练均未实现，不能作为当前能力宣传。
